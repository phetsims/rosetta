 ...
 */
const testTranslationInA11yView = async ( values: TranslationFormData, simName: string, locale: string ): Promise<void> => {
  const translation = await makeTranslationObject( values, simName, locale );
  try {
    const response = await fetch( `${TRANSLATION_API_ROUTE}/testTranslationInA11yView`, {
      method: 'POST',
      headers: {
